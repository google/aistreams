...
#ifndef AISTREAMS_UTIL_STATUS_STATUSOR_INTERNALS_H_
#define AISTREAMS_UTIL_STATUS_STATUSOR_INTERNALS_H_
...
class Helper {
 public:
  ...
  static void HandleInvalidStatusCtorArg(::aistreams::Status*);
  ABSL_ATTRIBUTE_NORETURN static void Crash(const ::aistreams::Status& status);
};
...
class StatusOrData {
 ...
 public:
  ...
  explicit StatusOrData(const ::aistreams::Status& status) : status_(status) {
    EnsureNotOk();
  }
  explicit StatusOrData(::aistreams::Status&& status)
  ...
  void Assign(const ::aistreams::Status& status) {
    Clear();
    status_ = status;
    EnsureNotOk();
  }

  void Assign(::aistreams::Status&& status) {
    Clear();
    status_ = std::move(status);
    EnsureNotOk();
  }
 ...
 protected:
  ...
  void MakeStatus(Args&&... args) {
    internal_statusor::PlacementNew<::aistreams::Status>(
...
#endif  // AISTREAMS_UTIL_STATUS_STATUSOR_INTERNALS_H_
